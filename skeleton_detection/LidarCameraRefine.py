...
from CameraLidarModule import CameraLidarModule, LidarKeypointAssociator, LabelCameraManager
...
from LidarFloorRemover import GroundPlaneRemover
...
def _rotation_to_euler(R):
    """Extract Euler angles (rx, ry, rz) in radians from R = Rz @ Ry @ Rx (same as _euler_to_rotation)."""
...
def _project_lidar_on_image(img, xyz, R, t, K, max_depth=5.0, image_opacity=0.6, crop_box=None):
    ...
    image_opacity: 0–1; lower values dim the image so points are easier to see (default 0.5).
    crop_box: optional (x_min, x_max, y_min, y_max, z_min, z_max) in lidar frame; only points
              inside this box are rendered. E.g. (-5, 5, -5, 5, -2, 2) for x in [-5,5], etc."""
    ...
    out = img.copy()
    # Dim the image so lidar points stand out (image_opacity: 0=full dim, 1=no dim)
    if image_opacity < 1.0:
        dark = np.full_like(out, 40, dtype=np.uint8)  # dark background
        out = cv2.addWeighted(out, image_opacity, dark, 1.0 - image_opacity, 0)
    for i in range(len(u)):
        cv2.circle(out, (int(u[i]), int(v[i])), 3,
                   (int(colors[i, 0]), int(colors[i, 1]), int(colors[i, 2])), -1)
    return out

def _rotate_lidar_y(xyz, angle):
    """Rotate lidar points around Y axis (same as Eigen example)."""
    c = np.cos(angle)
    s = np.sin(angle)
...
class SceneUI:
    ...
    def draw_extrinsic_panel(self):
        ...
        euler_deg = np.degrees(_rotation_to_euler(ext.R_opencv))
        ...
        for i, name in enumerate(["rx", "ry", "rz"]):
            if imgui.button(f"- ##neg_{name}"):
                ...
                self._extrinsic_changed = True
            imgui.same_line()
            imgui.text(f"{name}: {euler_deg[i]:+.2f} deg")
            imgui.same_line()
            if imgui.button(f"+ ##pos_{name}"):
                R_delta = _axis_rotation(i, r_step)
                ext.R_opencv = R_delta @ ext.R_opencv
...
class Viz:
    ...
    def __init__(self, width=900, height=700, title="3D Grid", dataset=None, icons=None):
        ...
        self._lidar_keypoint_assoc = LidarKeypointAssociator()
        self._skeletons_3d = []
        self._boxes_3d = []
        ...
        self.lidar_rotation_y = math.radians(10.0)

        self._ground_remover = GroundPlaneRemover(
            sensor_height=1.93,
            num_iter=15,
            num_lpr=400,
            th_seeds=0.3,
    ...
    def _overlay_lidar_on_images(self, images, xyz):
        ...
        for cam_idx, img in enumerate(images):
            ...
            result.append(_project_lidar_on_image(img, xyz, R, t, K, crop_box=(-20, 20, -30, 10, -6, 6)))
        return result
    ...
    def skeletons_to_3d(self, skeletons):
        ...
        for cam_entry in skeletons:
            ...
            cam_idx = cam_entry["cam"]

            for person in cam_entry.get("persons", []):
                ...
                for name, kp in person["keypoints"].items():
                    ...
                    P = self._lidar_keypoint_assoc.query(
                        cam_idx,
                        kp["x"],
                        kp["y"]
                    )
                ...
                if joints:
                    skeletons_3d.append(np.array(joints))

                # ------------------------------------------------
                # compute 2D bounding box from skeleton
                # ------------------------------------------------
                ...
                if xs and ys:
                    ...
                    box = self._lidar_keypoint_assoc.box2d_to_3d(
                        cam_idx,
                        bbox,
                        self._current_xyz,
                        self._dataset.extrinsics_array,
                        self._dataset.camera_array
                    )

                    if box is not None:
                        self._boxes_3d.append(box)

        return skeletons_3d

    def draw_skeletons(self):
        if not hasattr(self, "_skeletons_3d"):
            return

        # Small cube per keypoint (unit cube scaled down)
        keypoint_scale = 0.06
        S = scale(keypoint_scale, keypoint_scale, keypoint_scale)
        for skel in self._skeletons_3d:
            for p in skel:
                model = translate(p[0], p[1], p[2]) @ S
                self._set_model_color(model, 1.0, 0.2, 0.2)
                self._cube.draw()

    def draw_boxes(self):

        if not self._boxes_3d:
            return

        for center, size, yaw in self._boxes_3d:
            ...
            self._set_model_color(model, 0.1, 1.0, 0.2)
        ...
        while not glfw.window_should_close(self._window):
            ...
            if not self._ui._live_topic_mode:
                ...
                if self._dataset is not None and self._ui.consume_reload_flag():
                    ...
                    # rotate lidar
                    if abs(self.lidar_rotation_y) > 1e-9:
                        xyz = _rotate_lidar_y(xyz, self.lidar_rotation_y)
                    # remove floor
                    xyz = self._ground_remover.remove_ground(xyz)
                    # load skeletons for scene
                    self._ui._current_skeletons = self._dataset.load_skeleton(idx)
                    # update pointcloud
                    self._pointcloud.update(xyz)
                    ...
                    self._lidar_keypoint_assoc.build(
                        xyz,
                        self._dataset.extrinsics_array,
                        self._dataset.camera_array
                    )

                    self._skeletons_3d = self.skeletons_to_3d(self._ui._current_skeletons)
                ...
                if self._labels_dirty and self._path_images:
                    ...
                    if (self._ui.show_lidar_overlay and self._current_xyz is not None):
...
if __name__ == "__main__":
    ...
    dataset = SyncDataset(Path("../dataset-sdv-mar6-3"))
